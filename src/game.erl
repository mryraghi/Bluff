...
ready() ->
  receive
    {enter, PlayerName, No_Players} ->
      io:format("~p has entered the game!~n~p players.~n", [PlayerName, No_Players]),
      ready();
    {leave, PlayerName, No_Players} ->
      io:format("~p has left the game!~n~p players.~n", [PlayerName, No_Players]),
      ready()
  end.
  ...
  %% monitor the client
    {ServerPid, CtoS_Ref, {enter, PlayerName, ClientPid}} ->
      case orddict:find(ClientPid, S#state.players) of
        {ok, _E} ->
          ServerPid ! {CtoS_Ref, error, "You are already in the game!"},
          S#state.players,
          loop(S);
        error ->
...
enter(PlayerName, ClientPid) ->
  ...
  receive
    ...
    {CtoS_Ref, error, Reason} -> {error, Reason};
